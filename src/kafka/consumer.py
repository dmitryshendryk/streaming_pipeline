    ...
    def run(self, topic: str) -> None:
        ...
        def handler(message) -> None:
            ...
            if len(records) > 0:
                ...
                review_df = self.context.session.read.json(self.context.session.sparkContext.parallelize([review]))
                metadata_df = self.context.session.read.json(self.context.session.sparkContext.parallelize([metadata]))
